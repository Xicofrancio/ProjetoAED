...
UcTurma::UcTurma(std::string CodUc, std::string CodTurma) {
    codUc = CodUc;
    codTurma = CodTurma;
}
...
bool UcTurma::operator==(const UcTurma ucTurma) const {
    bool g = false;
    if (codUc == ucTurma.codUc && codTurma == ucTurma.codTurma) g = true;
    return g;
}
